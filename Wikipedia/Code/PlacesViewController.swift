...
class PlacesViewController: ArticleLocationCollectionViewController, UISearchBarDelegate, ArticlePopoverViewControllerDelegate, PlaceSearchSuggestionControllerDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate, ArticlePlaceViewDelegate, UIGestureRecognizerDelegate {
    ...
    @objc public func showCoordinate(latitude: Double, longitude: Double) {
        ...
        let locationDescription = String(format: "%.4f, %.4f", latitude, longitude)
        let location = CLLocation(latitude: latitude, longitude: longitude)
        
        currentSearch = PlaceSearch(filter: .top, type: .location, origin: .user, sortStyle: .links, string: nil, region: region, localizedDescription: locationDescription, searchResult: nil, siteURL: siteURL)
        mapRegion = region
    }
